    ...
    def NewRelatedQueries(self, ModeSave):
        ...
        try:
            ...
            for i in range(0, len(PalavraChave)): 
                ...
                LenInTop = len(list(related_queries_dict[QueryString]['top']["query"]))+1
                LenInRising = len(list(related_queries_dict[QueryString]['rising']["query"]))+1
                for i in (range(1, LenInTop)):
    ...
    def NewRelatedTopics(self, ModeSave): 
        ...
        try:
            ...
            for i in range(0, len(PalavraChave)): 
                ...
                LenInTop = len(list(related_topcs_dict[QueryString]['title']))+1
                for i in (range(1, LenInTop)):
                    ListValue['Top'].append(i)
                    ListValue['Termo Buscado'].append(QueryString)
                    ListValue['Data de Extração'].append(Time)
